...
final class FinanceController extends BaseController
{
    ...
    public function index(Request $request): Response
    {
        return $this->view('finance.index', [
            'summary'=>$this->finance->dashboard(),
            'invoices'=>$this->finance->invoices((string)$request->input('search','')),
        ]);
    }

    public function categories(Request $request): Response
    {
        return $this->view('finance.categories', ['categories'=>$this->finance->categories()]);
    }

    public function storeCategory(Request $request): Response
    {
        try { $this->finance->createCategory((string)$request->input('name',''),(string)$request->input('code',''),(string)$request->input('description','')); Session::flash('success','Fee category created.'); }
        catch(RuntimeException $e){ Session::flash('error',$e->getMessage()); }
        return $this->redirect('/finance/categories');
    }

    public function createInvoice(Request $request): Response
    {
        return $this->view('finance.invoice_create', [
            'students'=>$this->finance->students((string)$request->input('student_search','')),
            'categories'=>$this->finance->categories(), 'today'=>date('Y-m-d'), 'errors'=>[],
        ]);
    }
    ...
    {
        $items=[];
        $descriptions=(array)$request->input('description',[]); $quantities=(array)$request->input('quantity',[]); $amounts=(array)$request->input('unit_amount',[]); $categories=(array)$request->input('category_id',[]);
        ...
        try { $id=$this->finance->createInvoice(['student_id'=>$request->input('student_id'),'invoice_no'=>$request->input('invoice_no'),'invoice_date'=>$request->input('invoice_date'),'due_date'=>$request->input('due_date'),'discount'=>$request->input('discount'),'items'=>$items]); Session::flash('success','Invoice '.$request->input('invoice_no').' created.'); return $this->redirect('/finance?invoice='.$id); }
        catch(RuntimeException $e){ Session::flash('error',$e->getMessage()); return $this->redirect('/finance/invoices/create'); }
    }

    public function students(Request $request): Response
    { return Response::json(['success'=>true,'data'=>$this->finance->students((string)$request->input('search',''))]); }
    ...
    public function storePayment(Request $request): Response
    {
        ...
        catch(RuntimeException $e){ Session::flash('error',$e->getMessage()); return $this->redirect('/finance'); }
    }
}
