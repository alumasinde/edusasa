...
final class PaymentProviderService
{
    ...
    {
        $schoolId=\App\Core\Tenant::id();$studentId=(int)($input['student_id']??0);$channelId=(int)($input['channel_id']??0);$invoiceId=(int)($input['invoice_id']??0);$amount=round((float)($input['amount']??0),2);$phone=trim((string)($input['phone']??''));
    ...
    {
        $hash=hash('sha256',$token);$transaction=$this->repository->callbackTransaction($schoolId,$hash);if(!$transaction)throw new RuntimeException('Invalid payment callback.');
        $result=(array)($payload['Body']['stkCallback']??[]);$resultCode=(string)($result['ResultCode']??'');$resultDesc=(string)($result['ResultDesc']??'');$checkout=(string)($result['CheckoutRequestID']??'');
        $eventKey=$checkout!==''?$checkout:hash('sha256',json_encode($payload));if($this->repository->eventExists('mpesa',$eventKey))return;
        $eventId=$this->repository->recordEvent((int)$transaction['id'],'mpesa',$eventKey,$payload);if($eventId===0)return;
        try{$this->db->transaction(function()use($transaction,$result,$resultCode,$resultDesc,$payload,$eventId){
            ...
            $paidAmount=round((float)($meta['Amount']??0),2);$receipt=(string)($meta['MpesaReceiptNumber']??'');$phone=(string)($meta['PhoneNumber']??$transaction['phone']);
            if($receipt===''||$paidAmount<=0)throw new RuntimeException('M-Pesa callback did not contain a valid payment result.');
            if(abs($paidAmount-(float)$transaction['amount'])>0.01)throw new RuntimeException('M-Pesa callback amount does not match the payment request.');
            ...
            if(!empty($transaction['invoice_id'])){$invoice=$this->repository->invoice((int)$transaction['school_id'],(int)$transaction['student_id'],(int)$transaction['invoice_id']);if(!$invoice)throw new RuntimeException('The selected invoice is no longer payable.');$this->repository->allocate($paymentId,(int)$invoice['id'],$paidAmount);} 
