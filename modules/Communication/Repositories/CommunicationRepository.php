...
final class CommunicationRepository
{
    ...
    public function list(int $page=1,int $perPage=25):array{$offset=max(0,($page-1)*$perPage);return$this->db->select('SELECT c.*,CONCAT(u.first_name," ",u.last_name) sender_name FROM communications c LEFT JOIN users u ON u.id=c.sender_user_id WHERE c.school_id=:school_id ORDER BY c.created_at DESC LIMIT '.$perPage.' OFFSET '.$offset,['school_id'=>Tenant::id()]);}
    ...
    public function create(array $data):int{$data['school_id']??=Tenant::id();$columns=array_keys($data);$placeholders=[];$params=[];foreach($columns as $column){$placeholders[]=':'.$column;$params[$column]=$data[$column];}return(int)$this->db->insert('INSERT INTO communications('.implode(',',$columns).') VALUES('.implode(',',$placeholders).')',$params);}
    ...
    public function recipientsFor(array $message):array{
        ...
        if($message['audience_type']==='class'||$message['audience_type']==='stream'){$where='a.class_id=:class_id';$p=['school_id'=>Tenant::id(),'class_id'=>(int)$message['audience_class_id']];if($message['audience_type']==='stream'){$where.=' AND a.stream_id=:stream_id';$p['stream_id']=(int)$message['audience_stream_id'];}$sql='SELECT DISTINCT u.id user_id FROM users u JOIN teachers t ON t.school_id=u.school_id AND t.email IS NOT NULL AND t.email=u.email JOIN teacher_class_assignments a ON a.teacher_id=t.id AND a.school_id=u.school_id WHERE u.school_id=:school_id AND u.status="active" AND u.deleted_at IS NULL AND a.status="active" AND '.$where;return$this->db->select($sql,$p);}
