...
class GuardianService extends BaseService
{
    ...
    public function attachToStudent(int $studentId,array $data):int
    {
        ...
        $guardianId=(int)$this->db()->transaction(function()use($studentId,$data,$email,$phone){$id=$this->findOrCreateGuardian($email,$phone,$data);if(!empty($data['create_login']))$this->ensurePortalAccount($id,$email);$this->linkGuardianToStudent($studentId,$id,$data);return $id;});
    ...
    public function detach(int $studentGuardianId):void{$before=$this->studentGuardians->findOrFail($studentGuardianId);$this->studentGuardians->delete($studentGuardianId);AuditLog::record('guardian.detached','student_guardians',$studentGuardianId,$before,null);}
    ...
    private function findOrCreateGuardian(string $email,string $phone,array $data):int
    {
        ...
        return(int)$this->guardians->create(['first_name'=>$data['first_name'],'middle_name'=>($data['middle_name']??'')!==''?$data['middle_name']:null,'last_name'=>$data['last_name'],'email'=>$email!==''?$email:null,'phone'=>$phone!==''?$phone:null,'relationship'=>$data['relationship']??null,'occupation'=>($data['occupation']??'')!==''?$data['occupation']:null,'address'=>($data['address']??'')!==''?$data['address']:null,'national_id'=>($data['national_id']??'')!==''?$data['national_id']:null,'status'=>'active']);
    }
    ...
    {
        if($this->studentGuardians->existsForStudent($studentId,$guardianId))return;$primary=!empty($data['is_primary']);if($primary)$this->studentGuardians->clearPrimaryFlag($studentId);$this->studentGuardians->create(['student_id'=>$studentId,'guardian_id'=>$guardianId,'relationship'=>$data['relationship'],'is_primary'=>$primary?1:0]);
    }

    private function sendSetupEmailIfLinked(int $guardianId,string $email):void
    {
        $guardian=$this->guardians->findOrFail($guardianId);if($guardian['user_id']===null)return;$token=$this->authService->issuePasswordResetToken($email);if($token===null)return;$tenant=Tenant::current();$host=$tenant?->domain?:$tenant?->subdomain?:'edusasa.co.ke';$resetUrl='https://'.$host.'/reset-password?token='.$token;Mail::send($email,'Set up your '.($tenant?->name??'School').' parent portal account','<p>Click the link below to set your password.</p><p><a href="'.e($resetUrl).'">'.e($resetUrl).'</a></p><p>This link expires in 60 minutes.</p>');
    }
}
