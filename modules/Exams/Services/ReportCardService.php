...
final class ReportCardService
{
    ...
    public function generate(int $examId, ?int $studentId=null): int
    {
        ...
        $where=$studentId?' AND r.student_id=:student_id':'';
        $params=['school_id'=>$schoolId,'exam_id'=>$examId]; if($studentId)$params['student_id']=$studentId;
        ...
        $template=$this->db->selectOne('SELECT * FROM report_card_templates WHERE school_id=:school_id AND status="active" ORDER BY is_default DESC,id LIMIT 1',['school_id'=>$schoolId]);
        $count=0;
